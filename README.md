...
- **Importação de `.rsc` existente**: detecta e pré-preenche links WAN (inclusive por dedução, quando o arquivo não segue um padrão de nomenclatura específico), bridge LAN já existente, portas da bridge, **range original do DHCP pool**, **timezone original**, DDNS (MikroTik Cloud, mesmo sem hostname conhecido), grupos de bloqueio já configurados (inclusive no formato legado baseado em `tls-host`), e evita duplicar address-lists, `/ip dhcp-server` e blocos de timezone que já existem no arquivo — sem gerar comandos duplicados/conflitantes.
- **IP de Monitoramento opcional** com sugestões (1.1.1.1, 8.8.8.8, IPs de root servers) — PPPoE sem IP informado usa o mecanismo simples (gateway pelo nome da interface); DHCP sem IP informado usa 8.8.8.8 como padrão.
- **Tooltips** em praticamente todos os campos, com explicação curta e recomendação de uso.
...
Tema dark/tech, alinhado com [alctechsolutions.netlify.app](https://alctechsolutions.netlify.app/) — fundo escuro com grid e glows teal/azul, cartões em vidro fosco, ícones em gradiente teal/ciano e azul-escuro/azul-claro.
